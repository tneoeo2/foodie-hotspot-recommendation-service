...
class Restaurant(models.Model):
    # OPEN API
    sgg = models.CharField(max_length=100, null=True) # 시군구명
    sgg_code = models.CharField(max_length=100, null=True, blank=True) # 시군구 코드
    name = models.CharField(max_length=100, null=True) # 상호명
    ...
    address_lotno = models.CharField(max_length=200, null=True) # 지번주소
    address_roadnm = models.CharField(max_length=200, null=True) # 도로명주소
    zip_code = models.CharField(max_length=100, null=True, blank=True) # 우편번호
    longitude = models.CharField(max_length=100, default=0) # 경도
    latitude = models.CharField(max_length=100, default=0) # 위도
    ...
    def save(self, *args, **kwagrs):
        if self.name_address == None:
            if self.address_lotno != None:
                self.name_address = f"{self.name} {self.address_lotno}"
            else:
                self.name_address = f"{self.name} {self.address_roadnm}"
